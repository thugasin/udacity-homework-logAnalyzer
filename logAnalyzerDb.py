...
def get_top_popular(top_num):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT articles.title, COUNT(log.path) AS num FROM articles LEFT \
...
def get_top_author(top_num):
    conn = psycopg2.connect(database=DBNAME)
    cursor = conn.cursor()
    cursor.execute("SELECT authors.name,author_result.num FROM authors JOIN (SELECT \
                    SUM(article_result.num) as num, article_result.author \
                    from \
                    (SELECT articles.title, articles.author, COUNT(log.path)\
                    AS num FROM articles LEFT JOIN log ON log.path LIKE '%' \
                    || articles.slug || '%' GROUP BY articles.title, \
                    articles.author) AS article_result \
                    GROUP BY article_result.author)\
                    as author_result\
...
def get_show_stoper_days():
    conn = psycopg2.connect(database=DBNAME)
    cursor = conn.cursor()
    cursor.execute("SELECT time from (SELECT all_result.time::DATE, \
