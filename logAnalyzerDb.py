...
def get_top_author(top_num):
    conn = psycopg2.connect(database=DBNAME)
    cursor = conn.cursor()
    cursor.execute("""SELECT authors.name,author_result.num 
                    FROM authors JOIN 
...
def get_show_stoper_days():
    conn = psycopg2.connect(database=DBNAME)
    cursor = conn.cursor()
    cursor.execute("""SELECT time,error_report.bad*100/error_report.all::float as errors from (SELECT all_result.time::DATE,
